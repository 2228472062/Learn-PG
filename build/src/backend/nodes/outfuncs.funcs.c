...
static void
_outGraphLabelRef(StringInfo str, const GraphLabelRef *node)
{
	WRITE_NODE_TYPE("GRAPHLABELREF");

	WRITE_OID_FIELD(labelid);
	WRITE_LOCATION_FIELD(location);
}

static void
_outGraphPropertyRef(StringInfo str, const GraphPropertyRef *node)
{
	WRITE_NODE_TYPE("GRAPHPROPERTYREF");

	WRITE_STRING_FIELD(elvarname);
	WRITE_OID_FIELD(propid);
	WRITE_OID_FIELD(typeId);
	WRITE_INT_FIELD(typmod);
	WRITE_OID_FIELD(collation);
	WRITE_LOCATION_FIELD(location);
...
_outOnConflictExpr(StringInfo str, const OnConflictExpr *node)
{
	...
	WRITE_ENUM_FIELD(lockStrength, LockClauseStrength);
...
_outForPortionOfExpr(StringInfo str, const ForPortionOfExpr *node)
{
	...
	WRITE_NODE_FIELD(targetFrom);
	WRITE_NODE_FIELD(targetTo);
	WRITE_NODE_FIELD(targetRange);
	WRITE_OID_FIELD(rangeType);
	WRITE_BOOL_FIELD(isDomain);
	WRITE_NODE_FIELD(overlapsExpr);
	WRITE_NODE_FIELD(rangeTargetList);
	WRITE_OID_FIELD(withoutPortionProc);
	WRITE_LOCATION_FIELD(location);
	WRITE_LOCATION_FIELD(targetLocation);
}
...
static void
_outGraphPattern(StringInfo str, const GraphPattern *node)
{
	WRITE_NODE_TYPE("GRAPHPATTERN");

	WRITE_NODE_FIELD(path_pattern_list);
	WRITE_NODE_FIELD(whereClause);
}

static void
_outGraphElementPattern(StringInfo str, const GraphElementPattern *node)
{
	WRITE_NODE_TYPE("GRAPHELEMENTPATTERN");

	WRITE_ENUM_FIELD(kind, GraphElementPatternKind);
	WRITE_STRING_FIELD(variable);
	WRITE_NODE_FIELD(labelexpr);
...
static void
_outForPortionOfClause(StringInfo str, const ForPortionOfClause *node)
{
	WRITE_NODE_TYPE("FORPORTIONOFCLAUSE");

	WRITE_STRING_FIELD(range_name);
	WRITE_LOCATION_FIELD(location);
	WRITE_LOCATION_FIELD(target_location);
	WRITE_NODE_FIELD(target);
	WRITE_NODE_FIELD(target_start);
	WRITE_NODE_FIELD(target_end);
}
...
static void
_outJsonTablePlanSpec(StringInfo str, const JsonTablePlanSpec *node)
{
	WRITE_NODE_TYPE("JSONTABLEPLANSPEC");

	WRITE_ENUM_FIELD(plan_type, JsonTablePlanType);
	WRITE_ENUM_FIELD(join_type, JsonTablePlanJoinType);
	WRITE_STRING_FIELD(pathname);
	WRITE_NODE_FIELD(plan1);
	WRITE_NODE_FIELD(plan2);
	WRITE_LOCATION_FIELD(location);
}
...
static void
_outRepackStmt(StringInfo str, const RepackStmt *node)
{
	WRITE_NODE_TYPE("REPACKSTMT");

	WRITE_ENUM_FIELD(command, RepackCommand);
	WRITE_NODE_FIELD(relation);
	WRITE_STRING_FIELD(indexname);
	WRITE_BOOL_FIELD(usingindex);
	WRITE_NODE_FIELD(params);
}
...
static void
_outCreatePropGraphStmt(StringInfo str, const CreatePropGraphStmt *node)
{
	WRITE_NODE_TYPE("CREATEPROPGRAPHSTMT");

	WRITE_NODE_FIELD(pgname);
	WRITE_NODE_FIELD(vertex_tables);
	WRITE_NODE_FIELD(edge_tables);
}

static void
_outPropGraphVertex(StringInfo str, const PropGraphVertex *node)
{
	WRITE_NODE_TYPE("PROPGRAPHVERTEX");

	WRITE_NODE_FIELD(vtable);
	WRITE_NODE_FIELD(vkey);
	WRITE_NODE_FIELD(labels);
	WRITE_LOCATION_FIELD(location);
}

static void
_outPropGraphEdge(StringInfo str, const PropGraphEdge *node)
{
	WRITE_NODE_TYPE("PROPGRAPHEDGE");

	WRITE_NODE_FIELD(etable);
	WRITE_NODE_FIELD(ekey);
	WRITE_NODE_FIELD(esrckey);
	WRITE_STRING_FIELD(esrcvertex);
	WRITE_NODE_FIELD(esrcvertexcols);
	WRITE_NODE_FIELD(edestkey);
	WRITE_STRING_FIELD(edestvertex);
	WRITE_NODE_FIELD(edestvertexcols);
	WRITE_NODE_FIELD(labels);
	WRITE_LOCATION_FIELD(location);
}

static void
_outPropGraphLabelAndProperties(StringInfo str, const PropGraphLabelAndProperties *node)
{
	WRITE_NODE_TYPE("PROPGRAPHLABELANDPROPERTIES");
...
static void
_outPropGraphProperties(StringInfo str, const PropGraphProperties *node)
{
	WRITE_NODE_TYPE("PROPGRAPHPROPERTIES");
...
static void
_outAlterPropGraphStmt(StringInfo str, const AlterPropGraphStmt *node)
{
	WRITE_NODE_TYPE("ALTERPROPGRAPHSTMT");

	WRITE_NODE_FIELD(pgname);
	WRITE_BOOL_FIELD(missing_ok);
	WRITE_NODE_FIELD(add_vertex_tables);
	WRITE_NODE_FIELD(add_edge_tables);
	WRITE_NODE_FIELD(drop_vertex_tables);
	WRITE_NODE_FIELD(drop_edge_tables);
	WRITE_ENUM_FIELD(drop_behavior, DropBehavior);
	WRITE_ENUM_FIELD(element_kind, AlterPropGraphElementKind);
	WRITE_STRING_FIELD(element_alias);
	WRITE_NODE_FIELD(add_labels);
	WRITE_STRING_FIELD(drop_label);
	WRITE_STRING_FIELD(alter_label);
	WRITE_NODE_FIELD(add_properties);
	WRITE_NODE_FIELD(drop_properties);
}
...
_outPublicationAllObjSpec(StringInfo str, const PublicationAllObjSpec *node)
{
	...
	WRITE_NODE_FIELD(except_tables);
	WRITE_LOCATION_FIELD(location);
}
...
_outWaitStmt(StringInfo str, const WaitStmt *node)
{
	...
	WRITE_STRING_FIELD(lsn_literal);
	WRITE_NODE_FIELD(options);
}
...
_outPlannerGlobal(StringInfo str, const PlannerGlobal *node)
{
	...
	WRITE_UINT64_FIELD(default_pgs_mask);
...
_outPlannerInfo(StringInfo str, const PlannerInfo *node)
{
	...
	WRITE_STRING_FIELD(plan_name);
	WRITE_STRING_FIELD(alternative_plan_name);
...
_outRelAggInfo(StringInfo str, const RelAggInfo *node)
{
	...
	WRITE_BITMAPSET_FIELD(apply_agg_at);
...
_outAppendPath(StringInfo str, const AppendPath *node)
{
	...
	WRITE_FLOAT_FIELD(limit_tuples);
	WRITE_NODE_FIELD(child_append_relid_sets);
}
...
_outPlannedStmt(StringInfo str, const PlannedStmt *node)
{
	...
	WRITE_BITMAPSET_FIELD(resultRelationRelids);
	WRITE_NODE_FIELD(appendRelations);
	WRITE_NODE_FIELD(subplans);
	WRITE_NODE_FIELD(subrtinfos);
	WRITE_BITMAPSET_FIELD(rewindPlanIDs);
	WRITE_NODE_FIELD(rowMarks);
	WRITE_BITMAPSET_FIELD(rowMarkRelids);
	WRITE_NODE_FIELD(relationOids);
	WRITE_NODE_FIELD(invalItems);
	WRITE_NODE_FIELD(paramExecTypes);
	WRITE_NODE_FIELD(utilityStmt);
...
_outModifyTable(StringInfo str, const ModifyTable *node)
{
	...
	WRITE_ENUM_FIELD(onConflictLockStrength, LockClauseStrength);
	WRITE_NODE_FIELD(onConflictSet);
...
_outNestLoop(StringInfo str, const NestLoop *node)
{
	...
	WRITE_BITMAPSET_FIELD(join.ojrelids);
...
static void
_outSubPlanRTInfo(StringInfo str, const SubPlanRTInfo *node)
{
	WRITE_NODE_TYPE("SUBPLANRTINFO");

	WRITE_STRING_FIELD(plan_name);
	WRITE_UINT_FIELD(rtoffset);
	WRITE_BOOL_FIELD(dummy);
}

static void
_outElidedNode(StringInfo str, const ElidedNode *node)
{
	WRITE_NODE_TYPE("ELIDEDNODE");

	WRITE_INT_FIELD(plan_node_id);
	WRITE_ENUM_FIELD(elided_type, NodeTag);
	WRITE_BITMAPSET_FIELD(relids);
}
