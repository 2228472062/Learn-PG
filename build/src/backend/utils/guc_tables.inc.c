...
struct config_generic ConfigureNames[] =
{
...

	{
		...
		.long_desc = gettext_noop("-1 means log statement in full; 0 means log only an ellipsis."),
		.flags = GUC_UNIT_BYTE,
		.vartype = PGC_INT,
		._int = {
...

	{
		...
		.long_desc = gettext_noop("0 means not to emit these warnings."),
		.flags = GUC_UNIT_S,
		.vartype = PGC_INT,
		._int = {
...

	{
		...
		.long_desc = gettext_noop("-1 disables the timeout and waits for the receiver to catch up; 0 does not wait for the receiver to catch up."),
		.flags = GUC_UNIT_MS,
		.vartype = PGC_INT,
		._int = {
